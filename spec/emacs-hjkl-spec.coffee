...
describe "AtomicEmacs", ->
  ...
  describe "emacs-hjkl:backward-char", ->
    it "moves the cursor backward one character", ->
      @testEditor.setState("x[0]")
      atom.commands.dispatch @editorView, 'emacs-hjkl:backward-char'
    ...
    it "extends an active selection if the mark is set", ->
      ...
      atom.commands.dispatch @editorView, 'emacs-hjkl:set-mark'
      atom.commands.dispatch @editorView, 'emacs-hjkl:backward-char'
  ...
  describe "emacs-hjkl:forward-char", ->
    it "moves the cursor forward one character", ->
      @testEditor.setState("[0]x")
      atom.commands.dispatch @editorView, 'emacs-hjkl:forward-char'
  ...
  describe "emacs-hjkl:backward-sexp", ->
    it "moves all cursors backward one symbolic expression", ->
      @testEditor.setState("(aa bb)\naa [0]\n(bb cc)[1]\n")
      atom.commands.dispatch @editorView, 'emacs-hjkl:backward-sexp'
  ...
  describe "emacs-hjkl:forward-sexp", ->
    it "moves all cursors forward one symbolic expression", ->
      @testEditor.setState("[0]aa\n[1](bb cc)\n")
      atom.commands.dispatch @editorView, 'emacs-hjkl:forward-sexp'
  ...
  describe "emacs-hjkl:previous-line", ->
    it "moves the cursor up one line", ->
      @testEditor.setState("ab\na[0]b\n")
      atom.commands.dispatch @editorView, 'emacs-hjkl:previous-line'
  ...
  describe "emacs-hjkl:next-line", ->
    it "moves the cursor down one line", ->
      @testEditor.setState("a[0]b\nab\n")
      atom.commands.dispatch @editorView, 'emacs-hjkl:next-line'
      expect(@testEditor.getState()).toEqual("ab\na[0]b\n")
  ...
  describe "emacs-hjkl:backward-paragraph", ->
    it "moves back to an empty line", ->
      @testEditor.setState("a\n\nb\nc\n[0]d")
      atom.commands.dispatch @editorView, 'emacs-hjkl:backward-paragraph'
  ...
  describe "emacs-hjkl:forward-paragraph", ->
    it "moves forward to an empty line", ->
      @testEditor.setState("a\n[0]b\nc\n\nd")
      atom.commands.dispatch @editorView, 'emacs-hjkl:forward-paragraph'
      expect(@testEditor.getState()).toEqual("a\nb\nc\n[0]\nd")
  ...
  describe "emacs-hjkl:back-to-indentation", ->
    it "moves cursors forward to the first character if in leading space", ->
      @testEditor.setState("[0]  aa\n [1] bb\n")
      atom.commands.dispatch @editorView, 'emacs-hjkl:back-to-indentation'
      expect(@testEditor.getState()).toEqual("  [0]aa\n  [1]bb\n")
  ...
  describe "emacs-hjkl:backward-kill-word", ->
    ...
    describe "when there is a single cursor", ->
      ...
      describe "When emacs-hjkl.killToClipboard is off", ->
        beforeEach ->
          atom.config.set 'emacs-hjkl.killToClipboard', false
  ...
  describe "emacs-hjkl:kill-line", ->
    beforeEach ->
      atom.config.set 'emacs-hjkl.killWholeLine', false

    it "deletes from the cursor to the end of the line if there is text to the right", ->
      @testEditor.setState("aaa b[0]bb\nccc")
      atom.commands.dispatch @editorView, 'emacs-hjkl:kill-line'
  ...
  describe "emacs-hjkl:kill-line when killWholeLine option is on", ->
    beforeEach ->
      atom.config.set 'emacs-hjkl.killWholeLine', true
  ...
  describe "emacs-hjkl:kill-region", ->
    it "deletes the selected regions", ->
      @testEditor.setState("a(0)b[0]c d[1]e(1)f")
      atom.commands.dispatch @editorView, 'emacs-hjkl:kill-region'
  ...
  describe "emacs-hjkl:copy-region-as-kill", ->
    it "clears the selection without deleting text", ->
      @testEditor.setState("aaa b(0)b[0]b ccc")
      atom.commands.dispatch @editorView, 'emacs-hjkl:copy-region-as-kill'
  ...
  describe "emacs-hjkl:append-next-kill", ->
    it "sets the killing flag", ->
      @testEditor.setState("a[0]b")
      atom.commands.dispatch @editorView, 'emacs-hjkl:append-next-kill'
      expect(State.killing).toBe(true)
  ...
  describe "emacs-hjkl:yank", ->
    describe "when there is a single cursor", ->
      beforeEach ->
        @testEditor.setState("x[0]y")

      it "does nothing if the global kill ring is empty", ->
        atom.commands.dispatch @editorView, 'emacs-hjkl:yank'
  ...
  describe "emacs-hjkl:yank-pop", ->
    describe "when performed immediately after a yank", ->
      ...
      it "replaces the yanked text with successive kill ring entries", ->
        ...
        atom.commands.dispatch @editorView, 'emacs-hjkl:yank-pop'
  ...
  describe "emacs-hjkl:yank-shift", ->
    describe "when performed immediately after a yank", ->
      beforeEach ->
        @testEditor.setState("[0]")
      ...
      it "replaces the yanked text with successive kill ring entries", ->
        ...
        expect(@testEditor.getState()).toEqual("ab[0]")
        atom.commands.dispatch @editorView, 'emacs-hjkl:yank-shift'
        expect(@testEditor.getState()).toEqual("cd[0]")
        atom.commands.dispatch @editorView, 'emacs-hjkl:yank-shift'
        expect(@testEditor.getState()).toEqual("ef[0]")
  ...
  describe "emacs-hjkl:delete-horizontal-space", ->
    it "deletes horizontal space around all cursors", ->
      @testEditor.setState("a [0] b\n\t[1]\t")
      atom.commands.dispatch @editorView, 'emacs-hjkl:delete-horizontal-space'
      expect(@testEditor.getState()).toEqual("a[0]b\n[1]")
  ...
  describe "emacs-hjkl:delete-indentation", ->
    it "joins each cursor's current line with the previous one if at the start of the line", ->
      @testEditor.setState("a \n [0]b\nc \n [1]d")
      atom.commands.dispatch @editorView, 'emacs-hjkl:delete-indentation'
      expect(@testEditor.getState()).toEqual("a[0] b\nc[1] d")
  ...
  describe "emacs-hjkl:open-line", ->
    it "inserts a newline in front of each cursor", ->
      @testEditor.setState("a[0]b\nc[1]d")
      atom.commands.dispatch @editorView, 'emacs-hjkl:open-line'
      expect(@testEditor.getState()).toEqual("a[0]\nb\nc[1]\nd")
  ...
  describe "emacs-hjkl:just-one-space", ->
    it "replaces horizontal space around each cursor with a single space", ->
      @testEditor.setState("a [0] b\n\t[1]\t")
      atom.commands.dispatch @editorView, 'emacs-hjkl:just-one-space'
      expect(@testEditor.getState()).toEqual("a [0]b\n [1]")
  ...
  describe "emacs-hjkl:transpose-chars", ->
    it "transposes the current character with the one after it", ->
      @testEditor.setState("ab[0]cd")
      atom.commands.dispatch @editorView, 'emacs-hjkl:transpose-chars'
  ...
  describe "emacs-hjkl:transpose-words", ->
    it "transposes the current word with the one after it", ->
      @testEditor.setState("aaa b[0]bb .\tccc ddd")
      atom.commands.dispatch @editorView, 'emacs-hjkl:transpose-words'
  ...
  describe "emacs-hjkl:transpose-lines", ->
    it "transposes this line with the previous one, and moves to the next line", ->
      @testEditor.setState("aaa\nb[0]bb\nccc\n")
      atom.commands.dispatch @editorView, 'emacs-hjkl:transpose-lines'
  ...
  describe "emacs-hjkl:downcase-word-or-region", ->
    it "creates a single history entry for multiple changes", ->
      @testEditor.setState("[0]aA BB\nCC[1] DD EE[2]\nFF [3]")
      atom.commands.dispatch @editorView, 'emacs-hjkl:downcase-word-or-region'
      atom.commands.dispatch @editorView, 'core:undo'
      expect(@testEditor.getState()).toEqual("[0]aA BB\nCC[1] DD EE[2]\nFF [3]")
  ...
  describe "emacs-hjkl:upcase-word-or-region", ->
    it "creates a single history entry for multiple changes", ->
      @testEditor.setState("[0]Aa bb\ncc[1] dd ee[2]\nff [3]")
      atom.commands.dispatch @editorView, 'emacs-hjkl:upcase-word-or-region'
      atom.commands.dispatch @editorView, 'core:undo'
      expect(@testEditor.getState()).toEqual("[0]Aa bb\ncc[1] dd ee[2]\nff [3]")
  ...
  describe "emacs-hjkl:capitalize-word-or-region", ->
    it "creates a single history entry for multiple changes", ->
      @testEditor.setState("[0]aA bb\ncc[1] dd ee[2]\nff [3]")
      atom.commands.dispatch @editorView, 'emacs-hjkl:capitalize-word-or-region'
      atom.commands.dispatch @editorView, 'core:undo'
      expect(@testEditor.getState()).toEqual("[0]aA bb\ncc[1] dd ee[2]\nff [3]")
  ...
  describe "emacs-hjkl:mark-sexp", ->
    it "marks a symbol forward of the cursor", ->
      @testEditor.setState("a[0] bc ")
      atom.commands.dispatch @editorView, 'emacs-hjkl:mark-sexp'
      expect(@testEditor.getState()).toEqual("a[0] bc(0) ")
  ...
  describe "emacs-hjkl:mark-whole-buffer", ->
    it "marks the whole buffer, with the cursor at the beginning", ->
      @testEditor.setState(" [0] ")
      atom.commands.dispatch @editorView, 'emacs-hjkl:mark-whole-buffer'
  ...
  describe "emacs-hjkl:exchange-point-and-mark", ->
    it "exchanges all cursors with their marks", ->
      @testEditor.setState("[0]..[1].")
      atom.commands.dispatch @editorView, 'emacs-hjkl:set-mark'
      atom.commands.dispatch @editorView, 'emacs-hjkl:forward-char'
      expect(@testEditor.getState()).toEqual("(0).[0].(1).[1]")
      atom.commands.dispatch @editorView, 'emacs-hjkl:exchange-point-and-mark'
      expect(@testEditor.getState()).toEqual("[0].(0).[1].(1)")

  describe "emacs-hjkl:close-other-panes", ->
    it "should close all inactive panes", ->
      ...
      atom.commands.dispatch @editorView, 'emacs-hjkl:close-other-panes'
  ...
  describe "emacs-hjkl:scroll-up", ->
    it "does not crash on an empty editor", ->
      @testEditor.setState('')
      atom.commands.dispatch @editorView, 'emacs-hjkl:scroll-up'

  describe "emacs-hjkl:scroll-down", ->
    it "does not crash on an empty editor", ->
      @testEditor.setState('')
      atom.commands.dispatch @editorView, 'emacs-hjkl:scroll-up'
